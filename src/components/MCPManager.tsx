...
export const MCPManager: React.FC<MCPManagerProps> = ({
  ...
  const loadServers = async () => {
    try {
      ...
      setError(t('mcp.messages.loadError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleServerRemoved = (name: string) => {
    ...
    setToast({ message: t('mcp.messages.serverRemoved', { name }), type: "success" });
  };
  ...
  const handleImportCompleted = (imported: number, failed: number) => {
    ...
    if (failed === 0) {
      setToast({
        message: t('mcp.messages.importSuccess', { count: imported }),
        type: "success"
      });
    } else {
      setToast({
        message: t('mcp.messages.importPartial', { imported, failed }),
        type: "error"
      });
    }
  };

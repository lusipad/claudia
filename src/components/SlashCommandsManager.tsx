...
export const SlashCommandsManager: React.FC<SlashCommandsManagerProps> = ({
  ...
  const loadCommands = async () => {
    try {
      ...
      setError(t('settings.commands.loadError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : t('settings.commands.saveError'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : t('settings.commands.deleteError');
  ...
  const groupedCommands = filteredCommands.reduce((acc, cmd) => {
    const key = cmd.namespace
      ...
      : `${cmd.scope === 'project' ? t('settings.commands.projectCommands') : t('settings.commands.userCommands')}`;
    if (!acc[key]) {
      acc[key] = [];
    }
    acc[key].push(cmd);
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold">
            {scopeFilter === 'project' ? t('settings.commands.projectTitle') : t('settings.commands.title')}
          </h3>
          <p className="text-sm text-muted-foreground mt-1">
            {scopeFilter === 'project'
              ? t('settings.commands.projectSubtitle')
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <div className="relative">
            ...
            <Input
              placeholder={t('settings.commands.searchCommands')}
              value={searchQuery}
        ...
        {scopeFilter === 'all' && (
          <Select value={selectedScope} onValueChange={(value: any) => setSelectedScope(value)}>
            ...
            <SelectContent>
              <SelectItem value="all">{t('settings.commands.allCommands')}</SelectItem>
              <SelectItem value="project">{t('settings.commands.project')}</SelectItem>
              <SelectItem value="user">{t('settings.commands.user')}</SelectItem>
            </SelectContent>
          </Select>
        )}
      </div>
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {searchQuery
                ...
                : scopeFilter === 'project'
                  ? t('settings.commands.noProjectCommands')
                  : t('settings.commands.noCommandsCreated')}
            </p>
            {!searchQuery && (
              <Button onClick={handleCreateNew} variant="outline" size="sm" className="mt-4">
                {scopeFilter === 'project'
                  ? t('settings.commands.createFirstProject')
                  : t('settings.commands.createFirst')}
              </Button>
            )}
          </div>
        </Card>
                            ...
                            <div className="flex items-center gap-4 text-xs">
                              {command.allowed_tools.length > 0 && (
                                <span className="text-muted-foreground">
                                  {command.allowed_tools.length} {command.allowed_tools.length === 1 ? t('settings.commands.tools') : t('settings.commands.toolsPlural')}
                                </span>
                              )}
                              
                              {command.has_bash_commands && (
                                <Badge variant="outline" className="text-xs">
                                  {t('settings.commands.bash')}
                                </Badge>
                              )}
                              
                              {command.has_file_references && (
                                <Badge variant="outline" className="text-xs">
                                  {t('settings.commands.files')}
                                </Badge>
                              )}
                              ...
                              >
                                {isExpanded ? (
                                  <>
                                    ...
                                    {t('settings.commands.hideContent')}
                                  </>
                                ) : (
                                  <>
                                    <ChevronRight className="h-3 w-3" />
                                    {t('settings.commands.showContent')}
                                  </>
                                )}
                              </button>
                            </div>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingCommand ? t('settings.commands.editCommand') : t('settings.commands.createNewCommand')}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label>{t('settings.commands.scope')}</Label>
              <Select 
                  ...
                  {(scopeFilter === 'all' || scopeFilter === 'user') && (
                    <SelectItem value="user">
                      <div className="flex items-center gap-2">
                        ...
                        {t('settings.commands.userGlobal')}
                      </div>
                    </SelectItem>
                  )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>{t('settings.commands.namespace')}</Label>
                <Input
                  placeholder={t('settings.commands.namespacePlaceholder')}
            ...
            {/* Description */}
            <div className="space-y-2">
              <Label>{t('settings.commands.description')}</Label>
              <Input
            ...
            {/* Content */}
            <div className="space-y-2">
              <Label>{t('settings.commands.commandContent')}*</Label>
              <Textarea
                placeholder={t('settings.commands.contentPlaceholder')}
                value={commandForm.content}
                onChange={(e) => setCommandForm(prev => ({ ...prev, content: e.target.value }))}
                className="min-h-[150px] font-mono text-sm"
              />
              <p className="text-xs text-muted-foreground">
                {t('settings.commands.contentHelp')}
              </p>
            </div>

            {/* Allowed Tools */}
            <div className="space-y-2">
              <Label>{t('settings.commands.allowedTools')}</Label>
              ...
              <p className="text-xs text-muted-foreground">
                {t('settings.commands.allowedToolsHelp')}
              </p>
            </div>
            ...
            {!editingCommand && (
              <div className="space-y-2">
                <Label>{t('settings.commands.examples')}</Label>
            ...
            {commandForm.name && (
              <div className="space-y-2">
                <Label>{t('settings.commands.preview')}</Label>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t('settings.commands.deleteCommand')}</DialogTitle>
          </DialogHeader>

          <div className="space-y-4 py-4">
            <p>{t('settings.commands.deleteConfirm')}</p>
            {commandToDelete && (
              <div className="p-3 bg-muted rounded-md">
            ...
            <p className="text-sm text-muted-foreground">
              {t('settings.commands.deleteWarning')}
            </p>
          </div>

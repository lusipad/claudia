...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      if (err instanceof Error) {
        ...
        errorMessage = t("errors.failedToLoad");
      }
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t("agents.deletedSuccess"), type: "success" });
      await loadAgents();
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("agents.createdSuccess"), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t("agents.updatedSuccess"), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t("agents.exportedSuccess"), type: "success" });
    } catch (err) {
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t("agents.importedSuccess"), type: "success" });
      await loadAgents();
    } catch (err) {
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-heading-4 mb-2">{t("agents.noAgentsTitle")}</h3>
                    ...
                    {totalPages > 1 && (
                      <div className="mt-6 flex justify-center gap-2">
                        ...
                        <span className="flex items-center px-3 text-body-small">
                          {t("agents.pageInfo", { current: currentPage, total: totalPages })}
                        </span>
                        <Button
                          size="sm"
                          variant="outline"

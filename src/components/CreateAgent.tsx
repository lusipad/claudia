...
export const CreateAgent: React.FC<CreateAgentProps> = ({
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      setError(t("agents.agentName") + " " + t("common.required"));
      return;
    }

    if (!systemPrompt.trim()) {
      setError(t("agents.taskDescription") + " " + t("common.required"));
      return;
    }
    ...
    try {
      ...
      setError(isEditMode ? t("errors.failedToUpdate") : t("errors.failedToCreate"));
      setToast({
        message: isEditMode ? t("errors.failedToUpdate") : t("errors.failedToCreate"),
        type: "error"
      });
    } finally {
      setSaving(false);
      ...
      <div className="max-w-6xl mx-auto flex flex-col h-full">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-heading-1">
                  {isEditMode ? t("agents.editTitle") : t("agents.createTitle")}
                </h1>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-4">
            ...
            <Card className="p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-caption text-muted-foreground">{t("agents.agentName")}</Label>
                  <Input
                    id="name"
                    value={name}
                ...
                <div className="space-y-2">
                  <Label className="text-caption text-muted-foreground">{t("agents.agentIcon")}</Label>
                  <motion.div
                    whileTap={{ scale: 0.97 }}
                    transition={{ duration: 0.15 }}
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Zap className={cn(
                        "h-4 w-4",
                        model === "sonnet45" ? "text-primary" : "text-muted-foreground"
                      )} />
                      <div className="text-left">
                        <div className="text-body-small font-medium">{t("agents.modelClaudeSonnet45")}</div>
                        <div className="text-caption text-muted-foreground">{t("agents.modelClaudeSonnet45Desc")}</div>
                      </div>
                    </div>
                  </motion.button>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="text-body-small font-medium">{t("agents.modelClaude4Opus")}</div>
                        <div className="text-caption text-muted-foreground">{t("agents.modelClaude4OpusDesc")}</div>
                      </div>
                    </div>
                  </motion.button>
                </div>
            ...
            {/* Configuration */}
            <Card className="p-5">
              <h3 className="text-heading-4 mb-4">{t("common.configuration")}</h3>
              <div className="space-y-2">
                <Label htmlFor="default-task" className="text-caption text-muted-foreground">{t("agents.taskDescription")} ({t("common.optional")})</Label>
                <Input
                  id="default-task"
                  type="text"
            ...
            <Card className="p-5">
              <div className="mb-4">
                <h3 className="text-heading-4 mb-1">{t("common.systemPrompt")}</h3>
                <p className="text-caption text-muted-foreground">
